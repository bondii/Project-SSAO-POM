	...
	)
{
	if (isActive) {
		...
		bool redo_world_matrix = redo_all || (!hasWorld);

		if (redo_world_matrix) {
			world = (*parent_world) * getLocalMatrix();
			hasWorld = true;
		}

		/* Draw children */
		while (c) {
			c->updateWorldMatrix(&world, redo_world_matrix);
...
mat4f Transformable::getWorldMatrixInverse()
{
	...
	while (i >= 0 && list[i]->hasWorldInv) {
		pw = &list[i]->worldMatrixInverse;
		i--;
	}
