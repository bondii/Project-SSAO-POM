...
{
	u32 len = strlen(str);
	char *s = (char *) MemoryAllocateDebug((sizeof(char) * (len + 1)), MEM_METHOD_MALLOC_FREE, file, func, line);
	s[len] = '\0';
...
{
	char *s = (char *) MemoryAllocateDebug((sizeof(char) * (n + 1)), MEM_METHOD_MALLOC_FREE, file, func, line);
	s[n] = '\0';
...
bool parseFloats(
		u32			n,
		f32			*num,
		char		(*str)[RC_COMMAND_MAX_ARG_LEN]
	)
{
	u32 i;
