...
void RCInit()
{
	 ...
	 */
	shader = SceneGraph::createShaderProgram("OgreShader", 0, "LambertTexture.vs", "LambertTexture.fs", 0);
	 ...
	 */
	ogre->setValue("LightPosition", light->getWorldPosition());
	ogre->setValue("LightColor", light->getColor());
...
u32 RCUpdate()
{
	...
	/* Get mouse screen coordinates (in pixels) */
	vec2f mouse_pos = Platform::getMousePosition() * 0.002f;
	...
	if (keys[KeyW])	move += 0.1f;
	if (keys[KeyS])	move -= 0.1f;
	if (keys[KeyA])	strafe -= 0.1f;
	if (keys[KeyD])	strafe += 0.1f;
	...
	Renderer::clearColor(vec4f(0.1f, 0.1f, 0.1f, 1.0f));
