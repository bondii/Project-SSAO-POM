	...
	)
{
	...
	void *ptr = MemoryAllocate(s, method);
	...
	)
{
	...
	if (p[1] != method)
		REPORT_WARNING("Incorrect allocation/deallocation usage. Match malloc/realloc/free, new/delete and new[]/delete[]\n");
	total_mem -= p[0];
	if (total_mem < 0)
		REPORT_WARNING("More memory was free'd than allocated");
	total_blocks--;
	...
	)
{
	...
	p = MemoryAllocate(mem_size, MEM_METHOD_NEW_DELETE);
	...
	)
{
	...
	p = MemoryAllocate(mem_size, MEM_METHOD_NEW_DELETE_ARRAY);
...
{
	MemoryFree(ptr, MEM_METHOD_NEW_DELETE_ARRAY);
}
...
{
	MemoryFree(ptr, MEM_METHOD_NEW_DELETE);
}
...
#define LOC_HASH_SIZE			503
#define FREE_HASH_SIZE			503
	...
	)
{
	...
	while (loc) {
		if (line == loc->line && loc->file &&  strcmp(file, loc->file) == 0)
			break;
	...
	)
{
	...
	void *ptr = MemoryAllocateDebug(s, method, file, func, line);
	...
	)
{
	...
	p = MemoryAllocateDebug(mem_size, MEM_METHOD_NEW_DELETE,
	...
	)
{
	...
	p = MemoryAllocateDebug(mem_size, MEM_METHOD_NEW_DELETE_ARRAY, 
		mem_info_file, mem_info_func, mem_info_line);
...
{
	MemoryFreeDebug(ptr, MEM_METHOD_NEW_DELETE_ARRAY);
}
...
{
	MemoryFreeDebug(ptr, MEM_METHOD_NEW_DELETE);
}
