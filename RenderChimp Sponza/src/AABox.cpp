	...
	)
{
	...
	if (offset == -1 || n == 0) {
		minCorner = vec3f(0.0f, 0.0f, 0.0f);
		maxCorner = vec3f(0.0f, 0.0f, 0.0f);
		return;
	}
	...
	for (i = 0; i < n; i++) {
		...
		minCorner.x = f[0] < minCorner.x ? f[0] : minCorner.x;
		minCorner.y = f[1] < minCorner.y ? f[1] : minCorner.y;
		minCorner.z = f[2] < minCorner.z ? f[2] : minCorner.z;

		maxCorner.x = f[0] > maxCorner.x ? f[0] : maxCorner.x;
		maxCorner.y = f[1] > maxCorner.y ? f[1] : maxCorner.y;
		maxCorner.z = f[2] > maxCorner.z ? f[2] : maxCorner.z;
