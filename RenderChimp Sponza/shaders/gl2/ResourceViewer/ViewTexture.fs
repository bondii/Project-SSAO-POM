...
void main()
{
	...
	if (split != 0) {
		...
		if (t.x < 1.0 && t.y < 1.0)
			gl_FragColor.rgb = vec3(0.0, 0.0, textureColor.b);
		else if (t.x < 1.0 && t.y >= 1.0)
			gl_FragColor.rgb = vec3(textureColor.r, 0.0, 0.0);
		else if (t.x >= 1.0 && t.y < 1.0)
			gl_FragColor.rgb = textureColor.aaa;
		else
			gl_FragColor.rgb = vec3(0.0, textureColor.g, 0.0);
	} else {
		...
		if (channel == 8) {
			...
			//if ((channel & 8) != 0) {
				...
				if ((c.x < 0.5 && c.y < 0.5) || (c.x >= 0.5 && c.y >= 0.5))
					checker = 0.75;
				gl_FragColor.rgb = textureColor.a * gl_FragColor.rgb + (1.0 - textureColor.a) * checker;
			//}
		}
	}
